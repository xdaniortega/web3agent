...
Interactive agent builder: prompts for name, lets you pick actions/tools/dynamic ABI, funds the agent, registers on ERC-8004, and opens chat.
...
```typescript
import { TransferEthAction } from "web3agent-sdk"
...
```typescript
import { fetchContractAbiTool, callContractTool } from "web3agent-sdk"
...
| `ARBISCAN_API_KEY` | No | For dynamic contract tools — higher rate limits on Arbiscan API |
